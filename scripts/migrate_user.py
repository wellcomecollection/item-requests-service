...
def create_user(role_arn, user_pool_id, user):
    ...
    client.admin_create_user(
        ...
        UserAttributes=[{"Name": "custom:patronId", "Value": str(user["patronId"])}],
        TemporaryPassword=f"CHANGEIT!_{temporary_password}",
    )

    return {"username": user["username"], "password": temporary_password}
...
if __name__ == "__main__":
    try:
        ...
        sys.exit(
            f"Usage: {__file__} <SIERRA_ID> <SIERRA_SECRET> <USER_POOL_ID> <EMAIL>"
        )
